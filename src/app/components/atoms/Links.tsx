...
function Links({link,selected_id}:{link:linkType,selected_id:number | undefined}) {
  ...
  return (
    <div className='py-3 uppercase font-medium link w-1/2'>
        <a 
          href={`#${link.title}`} 
          key={link.id}
        ...
        >
          ...
          // "bg-slate-100 mt-3 mr-3 h-[1px] w-16  duration-150 ease-in-out"
            :
            'items-center justify-items-center mt-2 mr-3 h-[2px] w-8 bg-slate-500 hover:bg-slate-100'} id='point'></span>
            {link.title}
        </a>
    </div>
  )
